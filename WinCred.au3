...
Func _WinCred_CredRead(Const ByRef $sTargetName, Const ByRef $iType)
	...
	$mCredential.Persist = DllStructGetData($tCREDENTIALW, "Persist")
	$mCredential.Persist = DllStructGetData($tCREDENTIALW, "AttributeCount")
	...
	ConsoleWrite(BinaryToString($mCredential.CredentialBlob, $SB_UTF16LE) & @CRLF)
	ConsoleWrite($mCredential.Persist & @CRLF)
	ConsoleWrite($mCredential.AttributeCount & @CRLF)
